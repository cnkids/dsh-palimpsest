...
#   - token：优先用本项目专用变量 SONAR_TOKEN_DSH_PALIMPSEST，未设置时回退通用
...
if [ -n "${SONAR_TOKEN_DSH_PALIMPSEST:-}" ]; then
  export SONAR_TOKEN="$SONAR_TOKEN_DSH_PALIMPSEST"
fi
if [ -z "${SONAR_TOKEN:-}" ]; then
  echo "提示：未设置 SONAR_TOKEN_DSH_PALIMPSEST / SONAR_TOKEN，若服务端要求鉴权会扫描失败" >&2
fi
